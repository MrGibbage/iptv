...
// row anyway, so the search index is dropped and recreated on each ingest.
...
// Begins a full-replace ingest. The transaction is held open across async
// parser callbacks (better-sqlite3 statements are synchronous, so interleaved
// awaits between batches are safe on this single connection). Callers MUST
// call commit() or rollback().
export function beginReplaceIngest(): IngestHandle {
  const d = getDb()
  d.exec('BEGIN IMMEDIATE')
  try {
    d.exec('DELETE FROM programmes; DELETE FROM epg_channels; DROP TABLE IF EXISTS programmes_fts;')
    createFtsTable(d)
  } catch (err) {
    d.exec('ROLLBACK')
    throw err
  ...
  const insChannel = d.prepare(
    'INSERT OR REPLACE INTO epg_channels (id, display_name, icon) VALUES (?, ?, ?)',
  )
  const insProgramme = d.prepare(
    'INSERT INTO programmes (channel_id, start_ms, stop_ms, title, description) VALUES (?, ?, ?, ?, ?)',
  )
  const insFts = d.prepare(
    'INSERT INTO programmes_fts (rowid, title, description, channel_name) VALUES (?, ?, ?, ?)',
  )
